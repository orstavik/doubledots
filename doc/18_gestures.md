...
Gestures is a type of state machines that need to direct an orchestra of other reactions to DOM events. This is complex activities; it requires a lot of boiler plate code; and it is error prone. To avoid all this code, DoubleDots have implemented a special `class StateMachineAttr` that `extends Attr`. `StateMachineAttr` dramatically simplify coding gestures. And it dramatically improve their safety and reusability. 

In use, `StateMachineAttr`s are complex, multi-trigger, and often focus on dispatching events. `click` is simple `StateMachineAttr`. `swipe` is a complex `StateMachineAttr` with many possible implementations. But the example we start with is `hover:`.
...
## Example of `longpress`

>> convert this example into a `StateMachineAttr` based 
...
## Examples of `StateMachineAttr`s
...
### old text
4. **`StateMachineAttr`s** vs. plain statemachines: `StateMachineAttr`s are complex statemachines that ***dynamically spawn and remove*** other reactions. We call them "`StateMachineAttr`s" because they commonly monitor the state of user interaction and spawn UI event reactions, but they are not restricted to UI and UI events. The main thing is that they dynamically erect and take down event listeners during their life cycle. In the next chapter we will talk about `StateMachineAttr`s.

3. multiple triggers? yes. multiple triggers for dom events? yes. binary or multistate? doesn't matter. triggered by outside-dom occurrences too? doesn't matter. Controllable by outside reactions? doesn't matter. => `StateMachineAttr`. complex beast.
