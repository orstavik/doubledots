...
function processTopArg(dict, {prefix, args}){
  for (let type in dict){
    const [matcher, funcs = [], dictFuncs = []] = dict[type];
    try {
      if (prefix.match(matcher)){
        //if word startsWith `${type}-`, then we use that to set the prefix.
...
export function PrefixTable(dict) {
  ...
  return args => args.map(topFunc).reduce((a, b) => Object.assign(a, b), {});
}
