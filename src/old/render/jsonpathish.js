...
function topLevelParse(str) {
  ...
  //str2 doesn't have any strings with .,||&&=><!()[] characters
  const steps = str2.split(".").map(step => {
    const match = /^([^[]*)(\[([^]]*)\])?$/g.exec(step);
    if (!match)
      throw SyntaxError("bad step: ", step);
    const [_, kF, vF] = match;
    let allKeys, exactKeys, keyFilters;
    if (kF === "*" || kF === "")
      allKeys = true;
    else {
      keyFilters = kF.split(",").map(wild).filter(Boolean);
      exactKeys = keyFilters.filter(identifier).filter(Boolean);
    }
    const valueFilters = vF ? vF.split(",").map(vf => valueFilter(vf, stringMap)) : [];
    return { allKeys, exactKeys, keyFilters, valueFilters };
  });
  return filterStep.bind(null, steps);
}
